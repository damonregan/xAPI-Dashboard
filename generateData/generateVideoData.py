...
def genVideoStatement(student, verb, activity, time, score=None):
	stmt = {
		'actor': {
			'name': student.name,
			'mbox': 'mailto:'+student.email
...
def main():
	...
	numStudents = 0
	randomVideoCountMax = 30
	videoData = []
	
	inFile = open('videos.csv', 'r')

	for line in inFile:
		line = line.split(',')

		if int(len(line)) == 1:
			line.append(random.randint(1,randomVideoCountMax))
			
		numStudents += int(line[1])
	...
	currentVideoIndex = 0
	currentVideo = ""
	countOfCurrentFood = int(videoData[currentVideoIndex][1])
	...
	for i in range(1, numStudents+1):
		...
		if i > countOfCurrentFood:
			currentVideoIndex += 1
			countOfCurrentFood += int(videoData[currentVideoIndex][1])
		
		currentVideo = videoData[currentVideoIndex][0]
		
		xAPIstatements.append(genVideoStatement(myclass.students[i-1], "Completed", currentVideo, time))
