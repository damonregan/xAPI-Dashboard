...
def genNutritionStatement(student, verb, activity, time, calories, fat, protein, carbs, score=None):
	stmt = {
		'actor': {
			'name': student.name,
			'mbox': 'mailto:'+student.email
		...
		'object': {
			'id': 'http://myschool.edu/xapi/{}'.format(activity),
			'definition': {
				'name': activity
			}
		},
...
def main():
	...
	for line in inFile:
		foodItem = []
		foodItem.append(line)
		foodItem.append(random.randint(randomCaloriesMin, randomCaloriesMax))
		foodItem.append(random.randint(randomComponentMin, randomComponentMax))
		foodItem.append(random.randint(randomComponentMin, randomComponentMax))
		foodItem.append(random.randint(randomComponentMin, randomComponentMax))
		
		foodData.append(foodItem)
	
	myclass = Class(numStudents, 75, 20)

	numFoodItemsForStudent = 0
	numFoodItemsTotal = len(foodData)

	foodIndex = 0
	calories = 0
	
	xAPIstatements = []
	time = dt.datetime.now(EST())
	
	for i in range(1, numStudents+1):
		numFoodItemsForStudent = random.randint(1, randomFoodCountMax)
		
		for j in range(1, numFoodItemsForStudent): 
			time = dt.datetime.now(EST()) + dt.timedelta(seconds=i)
		
			foodIndex = random.randint(0, numFoodItemsTotal - 1)
			
			currentFood = foodData[foodIndex][0]
			calories 	= foodData[foodIndex][1]
			fat 		= foodData[foodIndex][2]		
			protein 	= foodData[foodIndex][3]
			carbs	 	= foodData[foodIndex][4]
		
			xAPIstatements.append(genNutritionStatement(myclass.students[i-1], "answered", currentFood, time, calories, fat, protein, carbs))

	stmtString = json.dumps(xAPIstatements, indent=4)
	...
	if '-o' in sys.argv:
		...
		try:
			with open(sys.argv[i+1],'w') as outfile:
				outfile.write("window.statements = " + stmtString)
		except IndexError:
			print stmtString
	else:
		print stmtString
